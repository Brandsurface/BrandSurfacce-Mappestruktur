...
(async () => {
  ...
  const isRetryable = !isOk && !!data.retryable;
  ...
  wrap.style.cssText =
    ...
    'color:#fff;max-width:320px;word-break:break-word;' +
    (isOk ? 'background:#16a34a;' : 'background:#dc2626;') +
    (isRetryable ? 'cursor:pointer;pointer-events:auto;' : 'pointer-events:none;') +
  ...
  text.textContent = data.message + (isRetryable ? ' (klik for at prøve igen)' : '');
  ...
  }, isOk ? 3000 : isRetryable ? 8000 : 5000);
  ...
  if (isRetryable) {
    wrap.addEventListener('click', () => {
      ...
      text.textContent = 'Åbner godkendelse…';
      chrome.runtime.sendMessage({ type: 'retry-permission' }).catch(() => {});
